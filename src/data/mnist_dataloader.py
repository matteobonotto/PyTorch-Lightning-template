...
class BaseDataModule(LightningDataModule):
    def __init__(
        ...
        self.datapipe_type = datapipe_type

    def setup(self, stage: Optional[str] = None):
        dps = DATAPIPE_REGISTRY[self.datapipe_type]
        self.train_dp = dps(path = self.path['train'])
        self.valid_dp = dps(path = self.path['val'])
        self.test_pd = dps(path = self.path['test'])
...
DATAPIPE_REGISTRY = {}
DATAPIPE_REGISTRY["BulkDataPipe"] = MNISTBulkDataset
DATAPIPE_REGISTRY["StreamDataPipe"] = MNISTStreamDatasetPdearena
